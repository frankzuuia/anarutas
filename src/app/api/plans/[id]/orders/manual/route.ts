import { assertOrderSource, persistManualImportPage } from "@/core/orders";
import { readFulfilledByOrderNames } from "@/core/odoo";
import { readOdooConfig } from "@/core/config";
import { AppError } from "@/core/errors";
import { vehicleIds } from "@/core/orders-validation";
...
export function POST(request: Request, ctx: Context) {
  return endpoint(async () => {
    ...
    if (!vehicleIds(input.vehicleIds).length)
      throw new AppError("SELECT_VEHICLES");
    ...
    return json(await persistManualImportPage(pool, user.id, id, page, input));
  });
}
