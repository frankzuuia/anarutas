...
internal fun friendlyError(error: Throwable): String = when (error) {
    is DriverApiException -> when (error.code) {
        ...
        "VISIT_NOT_ACTIVE" -> "Confirma una nueva llegada antes de atender este pedido."
        "ORDER_STATE_CONFLICT" -> "El pedido cambió de estado. Revisa su información actual antes de continuar."
        "RETRY_REQUIRES_NEW_ARRIVAL" -> "Usa Reintentar pedido y confirma una nueva llegada para atenderlo."
        "CUSTOMER_PHONE_INVALID" -> "Escribe un número válido, con código de país si corresponde."
        "CUSTOMER_PHONE_ALREADY_SET" -> "El cliente ya tiene un teléfono operativo. Se mostrará el número actualizado."
        "REJECTION_NOTE_REQUIRED" -> "Describe el motivo del rechazo en Otro."
        "INVALID_REJECTION_REASON" -> "Selecciona un motivo del rechazo."
        "INVALID_SERVICE_NOTE" -> "La nota debe tener como máximo 2,000 caracteres."
        "INCIDENT_CAPTURE_EXPIRED" -> "La evidencia local ya no está disponible y el servidor no confirmó el envío. Toma una foto nueva."
